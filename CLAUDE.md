...
A second export mode, **All Items** (suffix `- Full`), now sits alongside the outstanding export — the
export trigger is a dropdown. The full workbook lists every item per unit (including `S`-items) with a
